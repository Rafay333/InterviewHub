...
async function listLanguages(filters = {}) {
  ...
  if (filters.categoryId) {
    clauses.push("l.category_id = @categoryId");
  ...
  const result = await query(
    ...
    LEFT JOIN dbo.categories c ON c.id = l.category_id
...
async function createLanguage({
  ...
  const result = await query(
    `INSERT INTO dbo.languages
      (name, slug, description, picture_url, category_id, seo_heading, meta_title, meta_description, status, created_by)
     OUTPUT INSERTED.id
     VALUES (@name, @slug, @description, @picture_url, @category_id, @seo_heading, @meta_title, @meta_description, @status, @created_by)`,
    {
...
async function updateLanguage(id, { name, description, status, pictureUrl, categoryId }) {
  ...
  const nextCategoryId =
    categoryId !== undefined ? categoryId || null : current.categoryId || null;
  ...
  await query(
     ...
     WHERE id = @id`,
    {
      ...
      category_id: { type: sql.UniqueIdentifier, value: nextCategoryId },
      seo_heading: { type: sql.NVarChar(255), value: seoHeading },
