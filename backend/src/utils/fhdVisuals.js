/**
 * Diverse Full HD (1920×1080) explanation visuals — not 2/3-column bullet tables.
 * Layout kinds: steps | cycle | layers | code | timeline | hub | compare
 *
 * Optional theme: { accent, hubFill, hubText, centerFill, centerStroke, label }
 */
const fs = require("fs");
...
function save(buffer, slug) {
  ensureDir();
  const name = `${Date.now()}-${String(slug || "viz")
    .replace(/[^a-z0-9]+/gi, "-")
    .slice(0, 40)
    .toLowerCase()}-fhd.png`;
...
function titleBar(ctx, title, accent) {
  ctx.fillStyle = "#0b1f3a";
  ctx.font = "700 56px Segoe UI, Arial, sans-serif";
  const t = String(title || "Explanation");
  let size = 56;
...
function arrowRight(ctx, x1, y, x2, color) {
...
function arrowDown(ctx, x, y1, y2, color) {
  ctx.strokeStyle = color;
  ctx.fillStyle = color;
  ctx.lineWidth = 6;
  ctx.beginPath();
...
function drawSteps(ctx, contentTop, steps, accent) {
  ...
  const colors = [accent, "#0d9488", "#ea580c", "#7c3aed", "#db2777"];
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (i < n - 1) {
      arrowRight(ctx, x + boxW + 2, y + boxH / 2, x + boxW + gap - 2, accent);
    }
  }
}

function drawCycle(ctx, contentTop, items, theme, centerLabel) {
  const cx = W / 2;
  const cy = contentTop + (H - contentTop - 80) / 2;
  const R = 280;
  const n = Math.min(items.length, 6);
  const colors = [theme.accent, "#0d9488", "#ea580c", "#7c3aed", "#db2777", "#0891b2"];
  ...
  const mid = wrap(ctx, centerLabel || theme.label, 180);
  mid.forEach((ln, i) => {
...
function drawCode(ctx, contentTop, spec) {
  ...
  ctx.fillText(spec.file || "example", codeX + 120, codeY + 36);
...
function drawTimeline(ctx, contentTop, events, accent) {
  const xLine = 220;
...
function drawHub(ctx, contentTop, hub, nodes, theme) {
  const cx = W / 2;
  ...
  wrap(ctx, hub || theme.label, 200).forEach((ln, i) => {
    const tw = ctx.measureText(ln).width;
 ...
 * @returns {string|null}
 */
function renderVisual(visual, theme) {
  ...
  bg(ctx);
  const top = titleBar(ctx, visual.title, t.accent);
  const kind = visual.kind || "steps";

  if (kind === "steps") drawSteps(ctx, top, visual.steps || visual.items || [], t.accent);
  else if (kind === "cycle")
    drawCycle(ctx, top, visual.items || visual.steps || [], t, visual.center);
  else if (kind === "layers") drawLayers(ctx, top, visual.layers || visual.items || []);
  else if (kind === "code") drawCode(ctx, top, visual.code || {});
  else if (kind === "timeline")
    drawTimeline(ctx, top, visual.events || visual.items || [], t.accent);
  else if (kind === "hub") drawHub(ctx, top, visual.hub, visual.nodes || [], t);
  else if (kind === "compare") drawCompare(ctx, top, visual.left || {}, visual.right || {});
  else drawSteps(ctx, top, visual.steps || [], t.accent);

  footer(ctx, visual.footer || "");
  return save(canvas.toBuffer("image/png"), visual.title);
}

module.exports = { renderVisual, DEFAULT_THEME, W, H };
